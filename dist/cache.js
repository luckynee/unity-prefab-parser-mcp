 ...
 */
export async function findProjectRootAsync(filePath) {
    let current = path.dirname(path.resolve(filePath));
    const root = path.parse(current).root;
    while (current !== root) {
        const assetsPath = path.join(current, 'Assets');
 ...
 */
function getAssetType(metaContent, assetPath) {
 ...
 */
function extractGuid(metaContent) {
 ...
 */
export async function buildAssetCache(projectRoot, useCache = true) {
 ...
 */
export function resolveGuid(guid, cache) {
    return cache[guid] || null;
}
/**
 * Format an asset reference for output
 */
export function formatAssetReference(guid, cache, showType = true) {
