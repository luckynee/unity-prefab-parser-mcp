 ...
 */
export function formatYAML(data: ParsedPrefab, config: ParseConfig): string {
  ...
  const yamlOptions: YAML.DocumentOptions & YAML.SchemaOptions & YAML.ToStringOptions = {
 ...
 */
function formatHierarchy(nodes: HierarchyNode[], config: ParseConfig): unknown[] {
 ...
 */
function formatHierarchyNode(node: HierarchyNode, config: ParseConfig): Record<string, unknown> {
 ...
 */
function formatValue(value: unknown, config: ParseConfig): unknown {
 ...
 */
function isVectorLike(obj: Record<string, unknown>): boolean {
 ...
 */
function formatCompactObject(obj: Record<string, unknown>): Record<string, number> {
 ...
 */
export function formatYAMLWithComments(
  data: ParsedPrefab,
  config: ParseConfig
): string {
  const lines: string[] = [];
  ...
  if (data.variant_of && config.useTreeHierarchy) {
    const hasVariantContent = 
      (data.modifications && Object.keys(data.modifications).length > 0) ||
    ...
    if (data.added_components && Object.keys(data.added_components).length > 0) {
      ...
      for (const [goName, components] of Object.entries(data.added_components)) {
        ...
        if (typeof components === 'object' && components !== null) {
          for (const [compType, compData] of Object.entries(components as Record<string, unknown>)) {
            const marker = config.showVariantMarkers ? '  # +' : '';
            lines.push(`${indent}${indent}${compType}:${marker}`);
            formatObjectToLines(compData as Record<string, unknown>, lines, indent, 3, config);
          }
        }
      }
      lines.push('');
  ...
  if (!data.variant_of && Object.keys(data.components).length > 0) {
    ...
    for (const [gameObjectName, components] of Object.entries(data.components)) {
      ...
      if (typeof components === 'object' && components !== null) {
        for (const [componentType, componentData] of Object.entries(components as Record<string, unknown>)) {
          const compObj = componentData as Record<string, unknown>;
 ...
 */
function formatModificationsToLines(
 ...
 */
function canInlineComponent(obj: Record<string, unknown>): boolean {
  const keys = Object.keys(obj);
 ...
 */
function formatInlineComponent(obj: Record<string, unknown>, config: ParseConfig): string {
 ...
 */
function formatHierarchyToLines(
  node: HierarchyNode,
  lines: string[],
  indent: string,
  level: number,
 ...
 */
function formatParenVector(obj: Record<string, unknown>): string {
  const keys = Object.keys(obj);
  ...
  const values: string[] = [];
  if ('x' in obj) values.push(format(obj.x));
  if ('y' in obj) values.push(format(obj.y));
  if ('z' in obj) values.push(format(obj.z));
  if ('w' in obj) values.push(format(obj.w));
...
const UNITY_LAYER_NAMES: Record<number, string> = {
  0: 'Default',
  1: 'TransparentFX',
  2: 'Ignore Raycast',
  3: 'Layer3',
 ...
 *   └── Child2
 */
export function formatHierarchyAsTree(
  nodes: HierarchyNode[],
  config: ParseConfig
): string {
  const lines: string[] = [];
 ...
 */
function formatNodeMetadata(node: HierarchyNode, config: ParseConfig): string {
  const meta: string[] = [];
 ...
 */
export function formatVariantHierarchyAsTree(
 ...
 */
function formatVariantTreeNode(
  node: VariantHierarchyNode,
  lines: string[],
  prefix: string,
  isLast: boolean,
  config: ParseConfig
 ...
 */
function getMarkerString(marker: 'added' | 'modified' | 'removed' | undefined, config: ParseConfig): string {
 ...
 */
function buildVariantTree(data: ParsedPrefab, config: ParseConfig): string {
  const lines: string[] = [];
  
  // Track GameObjects and their change types
  const gameObjectChanges = new Map<string, Set<'modified' | 'added' | 'removed'>>();
  
  // Helper to add a change type for a GameObject
  const addChange = (goName: string, changeType: 'modified' | 'added' | 'removed') => {
 ...
 */
function getCombinedMarkerString(changes: Set<'modified' | 'added' | 'removed'>, config: ParseConfig): string {
  ...
  if (changes.has('modified')) markers.push('$');
  if (changes.has('added')) markers.push('+');
  if (changes.has('removed')) markers.push('-');
 ...
 */
function formatVariantComponentsSection(
  data: ParsedPrefab,
  lines: string[],
  indent: string,
  config: ParseConfig
  ...
  const gameObjectComponents = new Map<string, {
    modified: Map<string, Record<string, unknown>>;  // compType -> fields
    added: Map<string, Record<string, unknown>>;     // compType -> data
    removed: string[];                                // compType names
  }>();
  
  // Helper to ensure GameObject entry exists
  const ensureGO = (goName: string) => {
  ...
  if (data.added_components) {
    for (const [goName, components] of Object.entries(data.added_components)) {
      if (typeof components === 'object' && components !== null) {
        const go = ensureGO(goName);
        for (const [compType, compData] of Object.entries(components as Record<string, unknown>)) {
          go.added.set(compType, compData as Record<string, unknown>);
        }
      }
    }
  }
 ...
 */
function formatModificationsAsComponents(
  modifications: Record<string, Record<string, Record<string, unknown>>>,
  lines: string[],
  indent: string,
  config: ParseConfig
