...
export function resolveReferences(
  ...
  for (const [key, value] of Object.entries(data)) {
    ...
    if (config.convertBitmasks && isLayerMaskObject(value)) {
      const bits = (value as Record<string, unknown>).m_Bits as number;
...
export function resolveValue(
  ...
  if (typeof value === 'object') {
    return resolveObject(value as Record<string, unknown>, fileIdMap, assetCache, config, depth);
  }
...
function resolveArray(
  ...
  if (arr.length > config.arrayMaxElements) {
    const resolved = arr.slice(0, config.arrayMaxElements).map(item =>
      resolveValue(item, fileIdMap, assetCache, config, depth + 1, fieldName)
    );
 ...
 */
function extractArrayItemNames(arr: unknown[]): string[] {
  const names: string[] = [];
...
function resolveObject(
  ...
  if (config.convertBitmasks && isLayerMaskObject(obj)) {
    const bits = obj.m_Bits as number;
...
export function resolveReference(
  ...
  const fileID = ref.fileID as string | number;
  const guid = ref.guid as string | undefined;
  const type = ref.type as number | undefined;
 ...
 */
function isVectorOrColor(obj: Record<string, unknown>): boolean {
 ...
 */
function formatCompactVector(obj: Record<string, unknown>): Record<string, unknown> {
 ...
 */
function isDefaultValue(value: unknown, key: string): boolean {
  ...
  if (typeof value === 'object' && value !== null) {
    ...
    if ('x' in obj && 'y' in obj && 'z' in obj && 'w' in obj) {
      if (obj.x === 0 && obj.y === 0 && obj.z === 0 && obj.w === 1) return true;
    }
    
    // Default scale (1,1,1)
    if (key.toLowerCase().includes('scale')) {
      if (obj.x === 1 && obj.y === 1 && obj.z === 1) return true;
    }
    
    // Default color (1,1,1,1) white
    if ('r' in obj && 'g' in obj && 'b' in obj && 'a' in obj) {
      if (obj.r === 1 && obj.g === 1 && obj.b === 1 && obj.a === 1) return true;
    }
  }
 ...
 */
function convertToBoolean(value: unknown): unknown {
 ...
 */
function isEmptyUnityEvent(value: unknown): boolean {
  if (typeof value !== 'object' || value === null) {
    return false;
  }
  ...
  if ('m_PersistentCalls' in obj) {
    const calls = obj.m_PersistentCalls as Record<string, unknown>;
 ...
 */
function isUnityEvent(value: unknown): boolean {
  if (typeof value !== 'object' || value === null) {
    return false;
  }
 ...
 */
export function simplifyUnityEvent(value: unknown): unknown {
  if (typeof value !== 'object' || value === null) {
    return value;
  }
  ...
  const persistentCalls = obj.m_PersistentCalls as Record<string, unknown>;
  ...
  for (const call of calls) {
    if (typeof call === 'object' && call !== null) {
      ...
      const methodName = callObj.m_MethodName as string | undefined;
 ...
 */
function isEmptyStaticBatchInfo(value: unknown): boolean {
  if (typeof value !== 'object' || value === null) {
    return false;
  }
 ...
 */
function isLayerMaskObject(value: unknown): boolean {
  if (typeof value !== 'object' || value === null) {
    return false;
  }
 ...
 */
function convertBitmaskToLayers(bits: number): number[] | string {
  if (bits === 0) return [];
 ...
 */
export function extractScriptName(
  scriptRef: Record<string, unknown>,
  assetCache: MetaFileCache,
  config: ParseConfig
): string {
  const guid = scriptRef.guid as string | undefined;
